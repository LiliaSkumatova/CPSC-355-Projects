...
char_size = 1							
	...
	define(khafre_r, x20)					// Set macros
        define(cheops_r, x21)					
        define(pyramid_base_r, x27)				
        define(result_r, w28)					
...
newPyramid:
	...
	mov 	pyramid_base_r, x8				// make easier to read
        mov     w19, 0						
        str     w19, [pyramid_base_r, pyramid_origin + coord_x]	

        mov     w19, 0						
        str     w19, [pyramid_base_r, pyramid_origin + coord_y]	

        mov     w19, w2							//w19 = w2
        str     w19, [pyramid_base_r, pyramid_base + size_width]	// Store the value of w19 to width

        mov     w19, w3							// w19 = w3
        str     w19, [pyramid_base_r, pyramid_base + size_length]	// Store the value of w19 to length

        mov     w19, w4							// w19 = w4
        str     w19, [pyramid_base_r, pyramid_height]			// Store the value of w19 to height

        ldr     w19, [pyramid_base_r, pyramid_base + size_width]	// Load the width to w19
        ldr     w21, [pyramid_base_r, pyramid_base + size_length]	// Load the length to w21
        ldr     w22, [pyramid_base_r, pyramid_height]			// Load the height to w22
        mul     w19, w19, w21						// w19 = width * length
        mul     w19, w19, w22						// w19 = (width * length) * height
	mov	w6, 3				
	sdiv	w19, w19, w6						//w19 = ((width * length) * height)/3
        str     w19, [pyramid_base_r, pyramid_volume]			// Store to volume
